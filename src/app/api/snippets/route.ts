...
export async function POST(req: Request) {
  try {
    ...
    const slug =
      body.title.toLowerCase().replace(/\s+/g, "-").slice(0, 80) +
      "-" +
      uuidv4().slice(0, 6);

    const tags = Array.isArray(body.tags)
      ? [...new Set(body.tags as string[])]
      : [];
...
export async function PUT(req: Request) {
  try {
    ...
    const uniqueTags = Array.isArray(body.tags)
      ? [...new Set(body.tags as string[])]
      : [];

    let existingSnippet = null;
    if (body.id) {
      existingSnippet = await prisma.snippet.findFirst({
        ...
        tags: {
          ...
          create: uniqueTags.map((tagName) => ({
            tag: {
              connectOrCreate: {
                where: { name: tagName },
                create: { name: tagName, type: "language" },
              },
            },
          })),
        },
...
export async function DELETE(req: Request) {
  try {
    ...
    if (snippet.authorId !== userId) {
      return NextResponse.json(
        { error: "Forbidden - You don't own this snippet" },
        { status: 403 }
      );
    }
